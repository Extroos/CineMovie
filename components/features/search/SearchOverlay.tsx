...
import { searchMovies, searchTVShows, getPosterUrl } from '../../../services/tmdb';
...
export default function SearchOverlay({ onClose, onMovieClick, onShowResults }: SearchOverlayProps) {
    ...
    const delayDebounceFn = setTimeout(async () => {
      if (query.trim()) {
        ...
        try {
          ...
          // Map TV results to Movie interface
          const mappedTV = tvResults.map(tv => ({
              ...tv,
              title: (tv as any).name,
              releaseDate: (tv as any).firstAirDate,
              mediaType: 'tv'
          })) as unknown as Movie[];

          // Map Anime to Movie interface for unified display
          const mappedAnime = animeResults.map((a: any) => ({
              id: a.id, 
              title: a.title?.english || a.name,
              overview: a.description || 'Anime',
              posterPath: a.posterPath, // CORRECTED: Use posterPath which is already mapped to full URL
              backdropPath: a.backdropPath || a.posterPath,
              voteAverage: 0, 
          ...
          setSuggestions([...mappedAnime, ...movieResults, ...mappedTV]);
        } catch (error) {
  ...
  const handleSearch = (e?: React.FormEvent) => {
    ...
    onShowResults(query, suggestions.length > 0 ? suggestions : []);
  };
        ...
        {!query && !searching && (
          <div style={{ marginTop: '20px' }}>
            ...
            <div style={{
              ...
              {['Action', 'Comedy', 'Sci-Fi', 'Anime', 'Horror', 'Romance', 'Kids', 'Trending'].map(tag => (
                <button
                  key={tag}
                  onClick={() => {
                    triggerHaptic('light');
