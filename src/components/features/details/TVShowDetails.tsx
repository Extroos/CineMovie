...
function TVShowDetails({ show, onClose, onActorClick }: TVShowDetailsProps) {
  ...
  useEffect(() => {
    async function loadDetails() {
      ...
      try {
        ...
        const inMyList = await isInMyList(show.id, 'tv');
        setInList(inMyList);
  ...
  const handlePlayClick = async (episodeNum = 1) => {
    triggerHaptic('heavy');
    setSelectedEpisode(episodeNum);
    
    const url = await VidSrcService.getTVEmbed(fullShow.id, selectedSeason, episodeNum);
    setStreamUrl(url);
  ...
  const handleResumeClick = async () => {
    ...
    if (resumeEpisode) {
      ...
      const url = await VidSrcService.getTVEmbed(fullShow.id, resumeEpisode.season, resumeEpisode.episode);
      setStreamUrl(url);
                        ...
                        <div style={{ display: 'flex', gap: '12px', marginBottom: '1.5rem', flexWrap: 'wrap', alignItems: 'center' }}>
                           ...
                           <button
                             ...
                             style={{
                                ...
                                background: inList ? '#E50914' : 'rgba(255, 255, 255, 0.1)',
