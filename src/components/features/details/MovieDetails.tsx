...
function MovieDetails({ movie, onClose, onListUpdate, onActorClick }: MovieDetailsProps) {
  ...
  useEffect(() => {
    async function loadDetails() {
      ...
      try {
        ...
        const inMyList = await isInMyList(movie.id, 'movie');
        setInList(inMyList);
  ...
  const handlePlayClick = async () => {
    triggerHaptic('heavy');
    const url = await VidSrcService.getMovieEmbed(fullMovie.id);
    setStreamUrl(url);
    setShowPlayer(true);
  };
  ...
  return (
    <div 
      ...
      style={{
        ...
        bottom: 0,
        zIndex: 3000,
        background: 'rgba(10, 10, 10, 0.85)', // Darker, less reliance on blur for the main container
        overflowY: 'auto',
...
const MovieDetailsContent = memo(MovieDetails);
