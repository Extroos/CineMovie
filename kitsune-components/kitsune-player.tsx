...
function KitsunePlayer({
  ...
  const proxyUrl = useMemo(() => {
    const proxyBase = env("NEXT_PUBLIC_PROXY_URL") || "/api";
  ...
  const uri = useMemo(() => {
    ...
    // Safety: If it's already proxied by our server/index.js (e.g. starts with /proxy), prefix it with base
    if (firstSourceUrl.startsWith('/proxy')) {
        const proxyBase = env("NEXT_PUBLIC_PROXY_URL") || "/api";
        return `${proxyBase}${firstSourceUrl}`;
    }

    if (!referer) return firstSourceUrl; // Fallback to direct if no referer and not proxied

    try {
      const url = encodeURIComponent(firstSourceUrl);
      return `${proxyUrl}?url=${url}&referer=${encodeURIComponent(referer)}`;
    } catch (error) {
      console.error("Error constructing proxy URI:", error);
      return firstSourceUrl;
    }
  }, [episodeInfo, proxyUrl]);
  ...
  useEffect(() => {
    ...
    const trackOptions: any = (episodeInfo?.tracks ?? []).map((track) => {
      const referer = episodeInfo?.headers?.Referer;
      let url = `${proxyUrl}?url=${encodeURIComponent(track.url)}`;
      ...
      subOrDub === "sub"
        ? {
          url: (() => {
            ...
            const referer = episodeInfo?.headers?.Referer;
            let url = `${proxyUrl}?url=${encodeURIComponent(defaultTrack)}`;
            if (referer) url += `&referer=${encodeURIComponent(referer)}`;
            return url;
          })(),
          type: "vtt",
      ...
      customType: {
        m3u8: (
          ...
          if (Hls.isSupported()) {
            ...
            artPlayerInstance.hls = hls; // Attach to instance for plugins
        ...
        // Configure settings panel items
        {
          ...
          onSelect: function(item: any) {
            if (item.url && typeof item.url === "string") {
              // Safety: If it's already a full proxy URL (starts with /api/proxy or http), use it directly
              // Otherwise wrap it (but they should already be wrapped by trackOptions)
              const finalUrl = item.url.startsWith('/') || item.url.startsWith('http') 
                ? item.url 
                : `${proxyUrl}?url=${encodeURIComponent(item.url)}`;

              art.subtitle.switch(finalUrl, {
                name: item.html,
              });
